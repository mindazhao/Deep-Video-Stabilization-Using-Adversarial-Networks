...
import time
from lib.utils_en_de_coder_copy import *
...
if opt.use_gan:
    netD1 = define_D(5+3, opt.ndf, 'n_layers', n_layers_D=7, norm='batch', use_sigmoid=False)
    netD2 = define_D(5*3+3, opt.ndf, 'n_layers', n_layers_D=7, norm='batch', use_sigmoid=False)
...
def train(epoch, lr, list_stable, list_unstable, list_feature, list_adjacent, list_affine, list_epoch):
    ...
    optimizerD = optim.Adam([{'params': netD1.parameters()}, {'params': netD2.parameters()}], lr=lr,betas=(opt.beta1, 0.999))
    ...
    for i in range(batch_idxs):
        ...
        images1, images2 = next(iter_sample)

        images1 = images1.cuda()
        images2 = images2.cuda()


        image_stable1, image_unstable1, images_gan1 = pre_propossing(images1)
        image_stable2, image_unstable2, images_gan2  = pre_propossing(images2)
        ...
        fake,T= netG(image_unstable1)
        fake2, T2= netG(image_unstable2)
        ...
        if opt.use_gan and with_gan:
            ...
            fake_ab1=torch.cat([image_unstable1[:,0:5,:,:],fake],dim=1)
            pred_fake1 = netD1.forward(fake_ab1.detach())
            loss_d_fake1 = criterionGAN(pred_fake1, False)

            real_ab1 = torch.cat([image_unstable1[:,0:5, :, :], image_stable1], dim=1)
            pred_real1 = netD1.forward(real_ab1.detach())
            loss_d_real1 = criterionGAN(pred_real1, True)

            fake_ab2 = fake.view(-1,18,opt.input_size,opt.input_size)
            pred_fake2 = netD2.forward(fake_ab2.detach())
            loss_d_fake2 = criterionGAN(pred_fake2, False)

            real_ab2 = torch.cat([fake[0:5,:,:,:], image_stable1[5:6,:,:,:]], dim=0).view(-1,18,opt.input_size,opt.input_size)
            pred_real2 = netD2.forward(real_ab2.detach())
            ...
            # Combined loss
            loss_d = (loss_d_fake1 + loss_d_real1 + loss_d_fake2 + loss_d_real2) * 0.5
        ...
        if opt.use_gan and with_gan:
            ...
            loss_g_fake1 = criterionGAN(pred_fake1, True)

            fake_ab2 = fake.view(-1, 18, opt.input_size, opt.input_size)
            pred_fake2 = netD2.forward(fake_ab2)
            loss_g_fake2 = criterionGAN(pred_fake2, True)





        loss_mse = torch.mean(torch.abs(image_stable1 - fake))
        loss_vgg = generator_criterion(fake,image_stable1 )

        loss_mse2 = torch.mean(torch.abs(image_stable2 - fake2))
        loss_vgg2= generator_criterion(fake2, image_stable2)


        loss_g2=torch.mean(torch.abs(fake2 - fake))


        if opt.use_gan and with_gan:

            loss_g = (loss_mse + loss_vgg+loss_mse2 + loss_vgg2) +(loss_g_fake1+loss_g_fake2)/2+loss_g2*10
        else:
            loss_g = (loss_mse + loss_vgg+loss_mse2 + loss_vgg2)+loss_g2*10
        ...
        if i % 10 == 0:
            ...
            writer.add_scalar('scalar/adjacent', loss_g2, i + epoch * batch_idxs)
            if opt.use_gan and with_gan:
                writer.add_scalar('scalar/loss_d_fake1_g', loss_g_fake1, i + epoch * batch_idxs)
                ...
                writer.add_scalar('scalar/loss_d_real2', loss_d_real2, i + epoch * batch_idxs)
                writer.add_scalar('scalar/loss_d_fake2', loss_d_fake2, i + epoch * batch_idxs)

        if i%10==0:
            writer.add_image('image/unstable',vutils.make_grid(image_unstable1[0:3, 5:8, :, :], normalize=True,scale_each=True),i + epoch * batch_idxs)
            if opt.use_gan and with_gan:
                writer.add_image('image/fake1_gray',vutils.make_grid(fake[0:3,:,:,:], normalize=True,scale_each=True),i + epoch * batch_idxs)
            writer.add_image('image/stable_fake',vutils.make_grid(fake[0:3, 0:3, :, :], normalize=True, scale_each=True),i + epoch * batch_idxs)
...
def process():
    ...
    class_name = ['test','lowquality','Regular', 'QuickRotation', 'Running', 'Parallax', 'Crowd', 'Zooming']
    crop_ratio = 0.2
    num_sample = 20
    stride_sample = 15
    threshold = 0
    ...
    for ii in range(2,len(class_name)):
        ...
        index_sample_test = [31,25,19,13,7]
        ...
        for video_id in range(0,len(list_videos)):
            ...
            for i in range(period):
                history_frame.append(cv2.resize(img_init,size,interpolation=cv2.INTER_AREA))
            # for i in range(index_sample_test[0]):
            ...
            for i in range(int(num_frame)):
                ...
                time_sum1 = time.time()
                for j in range(len(index_sample_test)):
                        list_unstable.append(cv2.cvtColor(history_frame[period - index_sample_test[j]], cv2.COLOR_BGR2GRAY),)
                npp = np.array(list_unstable)
                images = npp[np.newaxis, :, :, :]
                images = torch.from_numpy(images)
                images = images.cuda().float()
                ...
                if ret == False:
                        continue
                img_unstable_resize = cv2.resize(img_unstable, size,interpolation=cv2.INTER_AREA)
                img_unstable_tensor = torch.from_numpy(img_unstable_resize[np.newaxis, :, :]).cuda().float() / 255 * 2 - 1
                images = torch.cat((images, img_unstable_tensor.permute(0,3,1,2)), 1)
                ...
                # img_unstable_resize = cv2.cvtColor(img_unstable_resize, cv2.COLOR_BGR2RGB)
                # list_unstable.append(img_unstable_resize)
                now = (torch.from_numpy(img_unstable[np.newaxis, :, :]).cuda().float() / 255 * 2 - 1).permute(0,3,1,2)#cv2.cvtColor(img_unstable, cv2.COLOR_BGR2RGB)
                # now = now[np.newaxis, :, :, :]
                # now = torch.from_numpy(now)
                # now = now.cuda().float()
                ...
                time1 = time.time()
                im_output,T= netG(images)
                torch.cuda.synchronize()
                time2 = time.time()

                for t in range(5):
                    grid = F.affine_grid(T[t], im_output.size()).permute(0,3,1,2)
                    ...
                    now = F.grid_sample(now, grid_resize)
                ...
                im_output = im_output[0, :, :, :].data.cpu().numpy()
                im_output = (im_output + 1) * 127.5
                im_output = im_output.transpose((1, 2, 0))
                # samples = samples.reshape((144, 256,-1))
                im_output = np.array(im_output.astype(np.uint8))
                # update history_frame
                # fake_gray=cv2.cvtColor(samples,cv2.COLOR_BGR2GRAY)
                ...
                unstable_resize = cv2.resize(img_unstable, (640, 360), interpolation=cv2.INTER_AREA)
                ...
                history_frame.append(im_output)
...
def main():
    if opt.train:
        ...
        for epoch in range(opt.continue_train, opt.nEpochs):
                ...
                lr = opt.lr * 0.1 ** int((epoch-opt.continue_train) / 10)
