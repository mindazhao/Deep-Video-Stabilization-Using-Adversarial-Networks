...
import cv2
import itertools
from lib.cfg_en_de_coder_copy import *
import numpy as np
...
def list_random(files):
    ...
    for video_id in range(0, len(files)):
        ...
        for i in range(period, numframe-1):
            video_frame = [video_id, i]
            list_random.append(video_frame)
        f.close()
    return list_random
...
class customData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image_unstable = self.list_unstable[video_id][
            numframe_id]  # cv2.imread(image_path_unstable + str(int(numframe_id)) + '.png')
        list_unstable.append(image_unstable)

        image_stable = self.list_stable[video_id][
            numframe_id]  # cv2.imread(image_path_stable + str(int(numframe_id)) + '.png')
        list_stable_unstable.append(image_stable)
        ...
        return img_data1, img_data2 # stable->unstable


def pre_propossing(images):
    ...
    return images_stable, images_unstable, images_gan
