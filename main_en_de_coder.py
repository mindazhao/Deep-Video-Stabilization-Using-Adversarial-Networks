...
import time
from lib.utils_en_de_coder import *
...
def train(epoch, lr, list_stable, list_unstable, list_feature, list_adjacent, list_affine, list_epoch):
    ...
    for i in range(batch_idxs):
        ...
        images1, features1, affine1 = next(iter_sample)

        images1 = images1.cuda()
        features1 = features1.cuda().float()
        affine1=affine1.cuda().float()

        image_stable1, image_unstable1, images_gan, feature_stable1, feature_unstable1 = pre_propossing(images1, features1)
        ...
        if opt.use_gan and with_gan:
            ...
            # Combined loss
            loss_d = (loss_d_fake1 + loss_d_real1)/2# + loss_d_real1 + loss_d_real2) * 0.5
        ...
        if opt.use_gan and with_gan:
            ...
            loss_g = (loss_mse + loss_vgg)*100  +(loss_g_fake1+loss_g_fake2)/2
        else:
            loss_g = (loss_mse + loss_vgg)
